...
bool get_tar_patch(int Select_modal, vector<float *> &vec_pkernels, vector<float *> &vec_pkernels_Hog, vector<float *> &vec_pkernels_map, vector<float *> &vec_pkernels_Hog_map,
	...
	float **** & p_img32f_tar_4d, long long &patch_size_all, map <int, float *> density_map_sub)
	...
	unsigned char * p_img_sub, map <int, float *> & density_map_sub)
{
	...
	if (input_Parameter.resample != 1 && input_Parameter.Select_modal==0)
	{
	...
	if (!get_tar_patch(input_Parameter.Select_modal, vec_pkernels, vec_pkernels_Hog, vec_pkernels_map, vec_pkernels_Hog_map, vec_corners, npixels_kernel,
		...
		sz_img_tar, p_img_label, p_img32f_tar_4d, patch_size_all,density_map_sub))
	{
			...
			if ((iter) % fre_region_constraint == 0)
			{
				...
				if (iter > input_Parameter.iterations_number / 2)
				{
					...
					for (int label_i = 0; label_i < label.size(); label_i++)
					{
						...
						if (input_Parameter.GPU_acceleration == 1)auto_warp_marker_gpu(lam1, stps_vec_tar, stps_vec_sub, stps_vec_sub_warp);//The GPU will miscalculate if the number of points is too low
...
//					printf("mark_q_1 ");
					...
					{
						if (!q_TPS_cd_gpu(stps_vec_tar, stps_vec_sub, lam, x4x4_affine, xnx4_c, xnxn_K))
						{
							printf("ERROR: (outline) q_TPS_cd_gpu() return false!\n");
							return false;
						}
					}
		...
		if (input_Parameter.Select_modal <2)
		{
			if ((iter) % int(input_Parameter.fre_global_constraint) == 0 && iter> input_Parameter.iterations_number / 2)
			{
				...
				if (GPU_acceleration == 1)
				{
					if (!q_TPS_cd_gpu(vec_corners, vec_corner_new, lam, x4x4_affine, xnx4_c, xnxn_K))
					{
						printf("ERROR: (inner all) q_TPS_cd_gpu() return false!\n");
						return false;
					}
				}
